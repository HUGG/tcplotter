...
def main():
    ...
    parser.add_argument('--cooling-rate-min', dest='cooling_rate_min',
                        help='Minimum cooling rate to plot in degrees C / Myr', default=0.1, type=float)
    parser.add_argument('--cooling-rate-slow', dest='cooling_rate_slow',
                        help='"Slow" cooling rate to plot in degrees C / Myr', default=1.0, type=float)
    parser.add_argument('--cooling-rate-avg', dest='cooling_rate_avg',
                        help='"Average" cooling rate to plot in degrees C / Myr', default=10.0,
                        type=float)
    parser.add_argument('--cooling-rate-max', dest='cooling_rate_max',
                        help='Maximum cooling rate to plot in degrees C / Myr', default=100.0, type=float)
    parser.add_argument('--temp-max', dest='temp_max', help='Maximum temperature for cooling history in degrees C',
                        default=350.0,
                        type=float)
    parser.add_argument('--time-max', dest='time_max',
    ...
    time_vs_temp(cooling_rate_min=args.cooling_rate_min, cooling_rate_slow=args.cooling_rate_slow,
                 cooling_rate_avg=args.cooling_rate_avg,
                 cooling_rate_max=args.cooling_rate_max, temp_max=args.temp_max, time_max=args.time_max,
                 save_plot=args.save_plot, plot_file_format=args.plot_file_format, plot_dpi=args.plot_dpi,
                 plot_style=args.plot_style, display_plot=display_plot
                 )
